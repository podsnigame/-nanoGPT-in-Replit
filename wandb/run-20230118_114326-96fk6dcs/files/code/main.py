  ...
  with torch.no_grad():
      ...
      with ctx:
          for k in range(num_samples):
              ...
              print(g)
              print('---------------')
  return model.train(), gen_list, start
...
if wandb_log and master_process:
    ...
    wandb_table = wandb.Table(columns=["iter_num", "start_text", "gen_1","gen_2","gen_3"])
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if wandb_log:
            model, gen_list, start = sample(model)

            wandb_table.add_data(iter_num, start, gen_list[0], gen_list[1], gen_list[2])              
            wandb.log({
                "iter": iter_num,
